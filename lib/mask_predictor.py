...
def get_norm_relu_layer(num_features, **kwargs):
...
class ProjectionHead(nn.Module):
    def __init__(self,
                 dim_in:   int = 1920, # 128+256+512+1024
                 proj_dim: int = 256,
                 bn_type:  str = 'torchbn'):
        super().__init__()
        ...
        self.proj = nn.Sequential(
            ...
            get_norm_relu_layer(dim_in),
            nn.Conv2d(dim_in, proj_dim, kernel_size=1, bias=False)
        )

    def forward(self, x):
        # (B, proj_dim, H, W) and pixel-wise L2-norm
        return F.normalize(self.proj(x), p=2, dim=1)

class SimpleDecoding(nn.Module):
    def __init__(self, c4_dims: int, factor: int = 2):
        ...
        in_43 = c4_size + c3_size
        self.conv1_4 = nn.Conv2d(in_43, hidden_size, 3, padding=1, bias=False)
        ...
        in_32 = hidden_size + c2_size
        self.conv1_3 = nn.Conv2d(in_32, hidden_size, 3, padding=1, bias=False)
