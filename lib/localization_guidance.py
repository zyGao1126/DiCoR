...
def token_valid_mask(input_ids: torch.Tensor, l_mask: torch.Tensor) -> torch.Tensor:
    valid = l_mask[:, :, 0].bool()
    return valid & (input_ids != 101) & (input_ids != 102)
...
def mask_centroid(mask_hw: torch.Tensor):
    ...
    if ys.numel() == 0:
        raise ValueError("Localization targets must contain foreground pixels")
...
def build_evidence_supervision(target: torch.Tensor, sam3_masks: Sequence[Sequence[torch.Tensor]], out_h: int, out_w: int):
    ...
    for batch_idx in range(target.shape[0]):
        ...
        center_y, center_x, area = mask_centroid(gt_hw)
...
def build_candidate_targets(candidates: Dict[str, torch.Tensor], sup: Dict[str, torch.Tensor], cfg: CandidateConfig):
    ...
    target_valid = target_valid & (target_dist <= float(cfg.gt_radius))
...
def compute_localization_loss(
    ...
    candidate_sup = build_candidate_targets(out, supervision, candidate_cfg)
    evidence_loss = compute_evidence_loss(out["evidence_logit"], supervision)
...
class CandidateGenerator:
    ...
    def _fill_candidate(
        ...
        outputs["cand_maps"][batch_idx, cand_idx] = raw / mass.clamp_min(1e-8)
    ...
    def __call__(self, prob_map: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        peak_mask = (prob_map >= pooled - 1e-12) & (prob_map >= 0.05)
        ...
        top_peak = outputs["peak_values"].max(dim=1, keepdim=True).values.clamp_min(1e-6)
        outputs["peak_rel"] = outputs["peak_values"] / top_peak
        return outputs


class LocalizationGuidanceModule(nn.Module):
    def __init__(self, cand_cfg: CandidateConfig, lambda_geo: float = 0.5):
        super().__init__()
        self.candidate_cfg = cand_cfg
        self.evidence_head = EvidenceHead(in_channels=512, hidden_channels=128)
    ...
    def forward(
        ...
        ranker_out = self.ranker(cand_visual, geom, text_tokens, token_valid.bool(), cand_valid)

        winner_prior = torch.einsum("bk,bkhw->bhw", ranker_out["cand_probs"], cand_maps).unsqueeze(1)
        winner_prior = winner_prior / winner_prior.flatten(2).sum(dim=-1).view(feature_map.shape[0], 1, 1, 1).clamp_min(1e-6)
...
class LocalizationGuidanceAdapter(nn.Module):
    def __init__(
        ...
        self.action_power = float(action_power)
    ...
    def forward(
        ...
        token_valid = token_valid_mask(input_ids, l_mask)
        ...
        action_map = guidance["cand_up"].pow(self.action_power)
        guidance["action_map"] = action_map
        enhanced = feature_map * (1.0 + self.alpha * action_map)
        return enhanced, guidance


def build_localization_guidance(alpha: float = 0.5, action_power: float = 4.0, area_threshold: float = 0.02, lambda_geo: float = 0.5) -> LocalizationGuidanceAdapter:
    cand_cfg = CandidateConfig()
    module = LocalizationGuidanceModule(cand_cfg, lambda_geo=lambda_geo)
    return LocalizationGuidanceAdapter(
        module=module,
        generator=CandidateGenerator(cand_cfg),
        alpha=alpha,
...
) -> LocalizationGuidanceAdapter:
    checkpoint = torch.load(ckpt_path, map_location=map_location)
    state = checkpoint["state_dict"] if isinstance(checkpoint, dict) and "state_dict" in checkpoint else checkpoint
    state = {key: value for key, value in state.items() if not key.startswith("text_norm.")}
    adapter = build_localization_guidance(alpha=alpha, lambda_geo=lambda_geo)
