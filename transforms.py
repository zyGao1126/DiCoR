    ...
    def __call__(self, image, target):
        ...
        if isinstance(target, dict):
            if "gold_masks" in target:
                target["gold_masks"] = F.resize(target["gold_masks"], (self.h, self.w), interpolation=F.InterpolationMode.NEAREST)
            if "sam3_masks" in target:
                target["sam3_masks"] = [
                    F.resize(m, (self.h, self.w), interpolation=F.InterpolationMode.NEAREST) 
                    for m in target["sam3_masks"]]
        else:
